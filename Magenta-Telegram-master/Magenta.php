	...
	*****************************************/
	...
	public static function checkUpdate( $offset = -1 ){
		...
		for ( $i = 0; $i < $max; $i++ ){
			...
			$rarr[$i]["upid"] = $arr["result"][0]["update_id"];
			$rarr[$i]["is_bot"] = $arr["result"][0]["message"]["from"]["is_bot"];
			$rarr[$i]["rawname"] = $arr["result"][0]["message"]["from"]["first_name"];
			$rarr[$i]["user_id"] = $arr["result"][0]["message"]["from"]["username"];
			$rarr[$i]["chat_id"] = $arr["result"][0]["message"]["chat"]["id"];
			$rarr[$i]["rawtitle"] = $arr["result"][0]["message"]["chat"]["title"];
			$rarr[$i]["type"] = $arr["result"][0]["message"]["chat"]["type"];
			$rarr[$i]["text"] = $arr["result"][0]["message"]["text"];
			$rarr[$i]["texttype"] = @$arr["result"][0]["message"]["entities"][0]["type"];
			$rarr[$i]["lang"] = @$arr["result"][0]["message"]["from"]["language_code"];
			
			} else if ( @$arr["result"][0]["message"]["chat"]["type"] == "private" ){
			...
			$rarr[$i]["type"] = $arr["result"][0]["message"]["chat"]["type"];
			$rarr[$i]["text"] = $arr["result"][0]["message"]["text"];
			$rarr[$i]["texttype"] = ( @$arr["result"][0]["message"]["entities"][0]["type"] !== null )? $arr["result"][0]["message"]["entities"][0]["type"] : null;
			$rarr[$i]["lang"] = $arr["result"][0]["message"]["from"]["language_code"];
			
			} else if ( null !== @$arr["result"][0]["callback_query"] ){
			
			$rarr[$i]["upid"] = $arr["result"][0]["update_id"];
			$rarr[$i]["cbtext"] = $arr["result"][0]["callback_query"]["data"];
			$rarr[$i]["cbmid"] = $arr["result"][0]["callback_query"]["message"]["message_id"];
			$rarr[$i]["cbcid"] = $arr["result"][0]["callback_query"]["message"]["chat"]["id"];
			$rarr[$i]["ikb"] = $arr["re sult"][0]["callback_query"]["message"]["reply_markup"];
			$rarr[$i]["texttype"] = "callback_query";
		...
		/*
		if ( null !== @$arr["result"][0]["inline_query"] ){
			
			$rarr[$i]["upid"] = $arr["result"][0]["update_id"];
			$rarr[$i]["is_bot"] = $arr["result"][0]["inline_query"]["from"]["is_bot"];
			$rarr[$i]["rawname"] = $arr["result"][0]["inline_query"]["message"]["from"]["first_name"];
			$rarr[$i]["user_id"] = $arr["result"][0]["inline_query"]["message"]["from"]["username"];
			$rarr[$i]["lang"] = $arr["result"][0]["inline_query"]["message"]["from"]["language_code"];
			$rarr[$i]["text"] = $arr["result"][0]["inline_query"]["query"];
	...
	*****************************************/
	...
	public static function Run(){
		...
		if ( $allow == true ) {
			...
			if ( isset ( $arr[$i]["texttype"] ) ) {

				if ( $arr[$i]["texttype"] == "bot_command" ){
							
							if ( ! Utils::isPRCommand( $arr[$i]["text"] ) ){
